...
onmessage = async function (event) {
  ...
  if(pars[0] == 'start') {
    ...
    var summing = {"axe": ['hammer', 'mailbox'], "bear": ["panda", "lion", "dog"], "door": ["cooler"], "tree": ['broccoli'], "hat": ["flying saucer", "helmet"], "camera": ["washing machine"],
     "lollipop": ["popsicle"], "house": ["barn", "hospital", "skyscraper"], "traffic light": ["streetlight"], "flower": ["windmill"], "shorts": ["pants"], "stairs": ["ladder"],
   "zigzag": ["lightning"], "butterfly": ["bat"], "cloud": ["bush"], "crown": ["grass"], "hockey stick": ["golf club", "clarinet"], "hand": ["arm"], "sock": ["foot", "shoe"]};
    let offset = tf.scalar(127.5);
    tf_arr = tf_arr.sub(offset).div(offset).reshape([-1, 64, 64, 1]);

    var predicted = model.predict(tf_arr).dataSync();
    var predi = [];
    for (var i = 0; i < predicted.length; i++){
      var clname = classes[i];
      var rsum = 0;
      if (clname in summing) {
        for (var j = 0; j < summing[clname].length; j++) {
          rsum += predicted[classes.indexOf(summing[clname][j])];
        }
      }
      console.log("Summing to " + classes[i] + " " + rsum + " " + predicted[i]);
      predi[i] = predicted[i] + rsum;
    }
    var cT = tf.tensor(predi);
    const {values, indices} = tf.topk(cT, 5);
